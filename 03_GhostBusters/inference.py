...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        #print "emission models : ", emissionModel
        pacmanPosition = gameState.getPacmanPosition()

        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        # to handle edge case when the Ghost is in jail and we need to update beliefs
        if noisyDistance is None:
            jailPosition = self.getJailPosition()
            allPossible[jailPosition] = 1
        # When the Ghost is not being eaten, then
        # P(True Distance between Pacman and the Ghost is x | Pacman has observed distance y) =
                # P(Pacman observing distance y | True Distance between Pacman and the Ghost is x) * P(True Distance between Pacman and Ghost is x) /
                # P(Pacman observing distance y)
        # where, P(Pacman observing the distance y) is evidence, hence it is equal to 1.
        #        P(Pacman observing distance y | True Distance between Pacman and the Ghost is x) is stored in emissionModel with observed distance as its key
        #        P(True Distance between Pacman and Ghost being x)  is stored in beliefs as the probability of Ghost being at a particular position
        else:
            for p in self.legalPositions:
                observedDistance = util.manhattanDistance(p, pacmanPosition)
                allPossible[p] = emissionModel[observedDistance] * self.beliefs[p]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # Dictionary to store newly calculated beliefs
        allPossible = util.Counter()

        # For each position in legal ghost positions at time t
        for oldPos in self.legalPositions:
            # Get probability distribution of positions Pi, which is:
            # Pr(ghost is at position Pi at time t + 1 | ghost is at position oldPos at time t)
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                # Pr(ghost is in newPos at time t + 1, ghost is in oldPos at time t) = Pr(ghost is in newPos at time t + 1 | ghost is in oldPos at time t) * Pr(ghost is in oldPos at time t)
                # Here:
                # self.beliefs[oldPos] = Pr(ghost is in oldPos at time t)
                #                 prob = Pr(ghost is in newPos at time t + 1 | ghost is in oldPos at time t)

                # We multiply these two terms, and add the result to the probability of newPos,
                # because there are multiple ways to reach newPos.
                allPossible[newPos] = allPossible[newPos] + (prob * self.beliefs[oldPos])

        # Normalize new probabilities and update beliefs
        allPossible.normalize()
        self.beliefs = allPossible
